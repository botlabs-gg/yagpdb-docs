...
| `complexMessageEdit "content" args "embed" args` | Special case for `editMessage` function, if `complexMessage` is involved, has two parameters `"content"` and `"embed"` to edit either regular text part of a complex message or embed part. Example in this section's [Snippets](templates.md#this-sections-snippets-6). |
